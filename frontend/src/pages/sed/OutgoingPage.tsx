...
export function OutgoingPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["outgoing", page, status],
    queryFn: () => sedApi.listOutgoing({ page, page_size: 20, status: status || undefined }),
  });

  return (
    <div>
      <PageHeader
        title="Журнал исходящих"
        subtitle={data ? `Всего: ${data.total}` : undefined}
        actions={
          <button className="btn-primary" onClick={() => navigate("/letters/new?type=outgoing")}>
            + Создать письмо
          </button>
        }
      />
      ...
      <div className="card mx-6 my-4">
        {isLoading ? (
          ...
          <EmptyState title="Исходящих писем нет" />
        ) : (
          <>
              ...
              <tbody className="divide-y divide-gray-50">
                {data.items.map((letter) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-gray-500 text-sm">{letter.to_company_id || "—"}</td>
